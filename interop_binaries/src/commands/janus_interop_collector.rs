...
async fn add_task_handler(
    ...
    request: Result<Json<AddTaskRequest>, axum::extract::rejection::JsonRejection>,
) -> impl IntoResponse {
...
async fn collection_start_handler(
    State(state): State<CollectorState>,
    request: Result<Json<CollectStartRequest>, axum::extract::rejection::JsonRejection>,
) -> impl IntoResponse {
    let Json(request) = match request {
        Ok(r) => r,
        Err(err) => {
...
async fn collection_poll_handler(
    State(state): State<CollectorState>,
    request: Result<Json<CollectPollRequest>, axum::extract::rejection::JsonRejection>,
) -> impl IntoResponse {
    let Json(request) = match request {
        Ok(r) => r,
        Err(err) => {
