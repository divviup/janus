...
use janus_messages::{
    batch_mode::BatchMode, BatchId, Duration, HpkeConfig, Interval, LeaderSelectedQuery,
...
enum ParsedQuery {
    ...
    LeaderSelected(LeaderSelectedQuery),
}
...
async fn handle_collection_start(
    ...
    let query = match request.query.batch_mode {
        ...
        2 => match request.query.subtype {
            Some(0) => {
                ...
                ParsedQuery::LeaderSelected(LeaderSelectedQuery::ByBatchId { batch_id })
            }
            Some(1) => ParsedQuery::LeaderSelected(LeaderSelectedQuery::CurrentBatch),
    ...
    let task_handle = match (query, vdaf_instance) {
        ...
        (ParsedQuery::LeaderSelected(leader_selected_query), VdafInstance::Prio3Count {}) => {
            let vdaf = Prio3::new_count(2).context("failed to construct Prio3Count VDAF")?;
            handle_collect_generic(
                http_client,
                task_state,
                Query::new_leader_selected(leader_selected_query),
                vdaf,
                &agg_param,
        ...
        #[cfg(feature = "fpvec_bounded_l2")]
        (
            ParsedQuery::LeaderSelected(leader_selected_query),
            janus_core::vdaf::VdafInstance::Prio3FixedPointBoundedL2VecSum {
                bitsize,
                dp_strategy: _,
                length,
        ...
        (ParsedQuery::LeaderSelected(leader_selected_query), VdafInstance::Prio3Sum { bits }) => {
            let vdaf = Prio3::new_sum(2, bits).context("failed to construct Prio3Sum VDAF")?;
            handle_collect_generic(
                http_client,
                task_state,
