                ...
                let err = tx
                    .query_one(
                        &format!(
                            "--
UPDATE tasks
SET aggregator_auth_token = {auth_token},
                ...
                let err = tx
                    .query_one(
                        &format!(
                            "--
UPDATE tasks
SET collector_auth_token_hash = {auth_token},
...
async fn check_other_report_aggregation_exists(ephemeral_datastore: EphemeralDatastore) {
    ...
    ds.run_unnamed_tx(|tx| {
        ...
        Box::pin(async move {
            assert!(tx
                .check_other_report_aggregation_exists::<0, dummy::Vdaf>(
                    &task_id,
                    &report_id,
                    &dummy::AggregationParam(0),
                    &random(),
            ...
            Box::pin(async move {
                ...
                let ok = tx
                    .query_one(
                        "--
SELECT (
        lower(interval) = '1972-07-21 05:30:00'
        AND upper(interval) = '1972-07-21 06:00:00'
        AND lower_inc(interval)
        AND NOT upper_inc(interval)
    ) AS ok
FROM (
        VALUES ($1::tsrange)
    ) AS temp (interval)",
                ...
                let ok = tx
                    .query_one(
                        "--
SELECT (
        lower(interval) = '2021-10-05 00:00:00'
        AND upper(interval) = '2021-10-06 00:00:00'
        AND lower_inc(interval)
        AND NOT upper_inc(interval)
    ) AS ok
FROM (
