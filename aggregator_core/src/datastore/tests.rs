...
async fn count_client_reports_for_interval(ephemeral_datastore: EphemeralDatastore) {
    ...
    let unrelated_task = TaskBuilder::new(task::QueryType::TimeInterval, VdafInstance::Fake)
        ...
        .leader_view()
        .unwrap();
    let no_reports_task = TaskBuilder::new(task::QueryType::TimeInterval, VdafInstance::Fake)
        .build()
        .leader_view()
        .unwrap();
            ...
            Box::pin(async move {
                ...
                Ok(Vec::from([ReportAggregation::new(
                    *task.id(),
                    ...
                    0,
                    None,
                    ReportAggregationState::<16, Poplar1<XofShake128, 16>>::StartLeader {
        ...
        Box::pin(async move {
            for task_id in &test_case.task_ids {
                tx.put_aggregator_task(
                    &TaskBuilder::new(test_case.query_type, VdafInstance::Fake)
                        .with_id(*task_id)
                        .build()
                        .leader_view()
                        .unwrap(),
...
async fn roundtrip_batch_aggregation_time_interval(ephemeral_datastore: EphemeralDatastore) {
    ...
    let task = TaskBuilder::new(task::QueryType::TimeInterval, VdafInstance::Fake)
        ...
        .leader_view()
        .unwrap();
    let other_task = TaskBuilder::new(task::QueryType::TimeInterval, VdafInstance::Fake)
        .build()
        .leader_view()
        .unwrap();
    ...
    let (
        ...
        .run_unnamed_tx(|tx| {
            Box::pin(async move {
                ...
                let outstanding_batch_1 = tx.get_outstanding_batch_with_report_count(&task_id_1, 1).await?;
                let outstanding_batch_2 = tx.get_outstanding_batch_with_report_count(&task_id_1, 2).await?;
                let outstanding_batch_3 = tx.get_outstanding_batch_with_report_count(&task_id_1, 3).await?;
