...
impl<C: Clock> Datastore<C> {
    ...
    #[cfg(feature = "test-util")]
    #[cfg_attr(docsrs, doc(cfg(feature = "test-util")))]
    pub async fn put_task(&self, task: &AggregatorTask) -> Result<(), Error> {
        self.run_tx(|tx| {
            let task = task.clone();
...
impl<C: Clock> Transaction<'_, C> {
    ...
    pub async fn get_task(&self, task_id: &TaskId) -> Result<Option<AggregatorTask>, Error> {
    ...
    #[tracing::instrument(skip(self), err)]
    pub async fn get_tasks(&self) -> Result<Vec<AggregatorTask>, Error> {
        let stmt = self
            .prepare_cached(
