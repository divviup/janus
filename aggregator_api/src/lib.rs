...
async fn post_task<C: Clock>(
    ...
    let task = Arc::new(
        Task::new(
            /* task_id */ random(),
            /* leader_aggregator_endpoint */ req.leader_aggregator_endpoint,
            /* helper_aggregator_endpoint */ req.helper_aggregator_endpoint,
...
mod models {
    ...
    pub(crate) struct PostTaskReq {
        pub(crate) leader_aggregator_endpoint: Url,
        pub(crate) helper_aggregator_endpoint: Url,
    ...
    impl From<&Task> for TaskResp {
        fn from(task: &Task) -> Self {
            ...
            Self {
                task_id: *task.id(),
                leader_aggregator_endpoint: task.leader_aggregator_endpoint().clone(),
                helper_aggregator_endpoint: task.helper_aggregator_endpoint().clone(),
...
mod tests {
    ...
    async fn post_task() {
        ...
        let req = PostTaskReq {
            leader_aggregator_endpoint: "http://leader.endpoint".try_into().unwrap(),
            helper_aggregator_endpoint: "http://helper.endpoint".try_into().unwrap(),
        ...
        assert_eq!(
            &req.leader_aggregator_endpoint,
            got_task.leader_aggregator_endpoint()
        );
        assert_eq!(
            &req.helper_aggregator_endpoint,
    ...
    fn post_task_req_serialization() {
        assert_tokens(
            &PostTaskReq {
                leader_aggregator_endpoint: "https://example.com/".parse().unwrap(),
                helper_aggregator_endpoint: "https://example.net/".parse().unwrap(),
