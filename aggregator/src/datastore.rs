...
impl<C: Clock> Transaction<'_, C> {
        ...
        let stmt = self
            ...
            .prepare_cached(
                "INSERT INTO tasks (task_id, aggregator_role, aggregator_endpoints, query_type,
                    vdaf, max_batch_query_count, task_expiration, report_expiry_age, min_batch_size,
        ...
        for<'a> &'a A::AggregateShare: Into<Vec<u8>>,
    {
        ...
        self.tx
            ...
            .map(|row| {
                ...
                Self::client_report_from_row(vdaf, *task_id, report_id, row)
            })
            .collect()
    }
        ...
        for<'a> &'a A::AggregateShare: Into<Vec<u8>>,
    {
        ...
        self.tx
            ...
            .into_iter()
            .map(|row| {
                let aggregation_job_id_bytes: [u8; AggregationJobId::LEN] = row
                    .get::<_, Vec<u8>>("aggregation_job_id")
                    .try_into()
                    .map_err(|err| {
                        Error::DbState(format!(
                            "couldn't convert aggregation_job_id value: {err:?}"
                        ))
                    })?;
                let aggregation_job_id = AggregationJobId::from(aggregation_job_id_bytes);
...
mod tests {
    ...
    use janus_messages::{
        query_type::{FixedSize, QueryType, TimeInterval},
        AggregationJobId, Duration, Extension, ExtensionType, HpkeCiphertext, HpkeConfigId,
