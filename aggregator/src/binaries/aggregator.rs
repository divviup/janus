...
pub struct Config {
    ...
    /// ECPrivateKey. Currently only P-256 keys are supported.
...
impl Config {
    ...
    fn aggregator_config(&self) -> Result<aggregator::Config> {
...
mod tests {
    ...
    use crate::{
        aggregator::{self, test_util::hpke_config_signing_key},
        config::{
    ...
    fn roundtrip_config(#[case] aggregator_api: AggregatorApi) {
        roundtrip_encoding(Config {
            ...
            hpke_config_signing_key: Some("hpke_config_signing_key".to_string()), // key is not parsed during serde
        })
    }
    ...
    fn config_hpke_signing_key() {
        // Key generated via `openssl ecparam -name prime256v1 -genkey -noout -out -`. Do not use
        // for production purposes. Matches the key generated by hpke_config_signing_key().
