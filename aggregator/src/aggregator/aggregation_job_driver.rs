            ...
            stepped_aggregations.iter().zip(helper_prep_resps)
        {
            ...
            let new_state = (|| match helper_prep_resp.result() {
                ...
                PrepareStepResult::Finished => {
                    if let ping_pong::State::Finished(output_share) =
...
mod tests {
    ...
    use janus_messages::{
        ...
        Interval, PartialBatchSelector, PlaintextInputShare, PrepareContinue, PrepareError,
        ...
        let collection_job = ds
            .run_tx(|tx| {
                ...
                Box::pin(async move {
                    ...
                    tx.put_batch(
                        &Batch::<VERIFY_KEY_LEN, TimeInterval, Poplar1<PrgSha3, 16>>::new(
                            *task.id(),
                            batch_identifier,
                            aggregation_param.clone(),
                            BatchState::Closing,
                    ...
                    let collection_job =
                        CollectionJob::<VERIFY_KEY_LEN, TimeInterval, Poplar1<PrgSha3, 16>>::new(
                            *task.id(),
                            random(),
                            batch_identifier,
                            aggregation_param.clone(),
    ...
    async fn step_time_interval_aggregation_job_init_single_round() {
        ...
        let want_batch = Batch::<VERIFY_KEY_LEN, TimeInterval, Prio3Count>::new(
            *task.id(),
            batch_identifier,
            (),
            BatchState::Closing,
    ...
    async fn step_fixed_size_aggregation_job_init_single_round() {
        ...
        let lease = ds
            .run_tx(|tx| {
                ...
                Box::pin(async move {
                    ...
                    tx.put_batch(&Batch::<VERIFY_KEY_LEN, FixedSize, Prio3Count>::new(
                        *task.id(),
                        batch_id,
                        (),
                        BatchState::Open,
        ...
        let want_batch = Batch::<VERIFY_KEY_LEN, FixedSize, Prio3Count>::new(
            *task.id(),
            batch_id,
            (),
            BatchState::Open,
        ...
        let lease = ds
            .run_tx(|tx| {
                ...
                Box::pin(async move {
                    ...
                    tx.put_batch(
                        &Batch::<VERIFY_KEY_LEN, FixedSize, Poplar1<PrgSha3, 16>>::new(
                            *task.id(),
                            batch_id,
                            aggregation_param.clone(),
                            BatchState::Open,
    ...
    async fn step_time_interval_aggregation_job_continue() {
        ...
        let want_active_batch = Batch::<VERIFY_KEY_LEN, TimeInterval, Poplar1<PrgSha3, 16>>::new(
            *task.id(),
            active_batch_identifier,
            aggregation_param.clone(),
            BatchState::Closed,
        ...
        let want_other_batch = Batch::<VERIFY_KEY_LEN, TimeInterval, Poplar1<PrgSha3, 16>>::new(
            *task.id(),
            other_batch_identifier,
            aggregation_param.clone(),
            BatchState::Closing,
        ...
        let (lease, collection_job) = ds
            .run_tx(|tx| {
                ...
                Box::pin(async move {
                    ...
                    let collection_job =
                        CollectionJob::<VERIFY_KEY_LEN, FixedSize, Poplar1<PrgSha3, 16>>::new(
                            *task.id(),
                            random(),
                            batch_id,
                            aggregation_param,
    ...
    async fn abandon_failing_aggregation_job() {
        ...
        assert_eq!(
            got_batch,
            Batch::<VERIFY_KEY_LEN, TimeInterval, Prio3Count>::new(
                *task.id(),
                batch_identifier,
                (),
                BatchState::Open,
