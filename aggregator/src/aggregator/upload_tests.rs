...
async fn upload_report_unsorted_extensions() {
    ...
    //
    // TODO: Update this test if we ever implement support for a second report extension type.
    // Lack of support for the reserved extension type could mask the error related to the order
    // of extension types.
    let report = create_report_custom(
        &task,
        clock.now().to_time(task.time_precision()),
...
async fn upload_report_unrecognized_extension() {
    for extension_type in [ExtensionType::Unknown(0xABCD), ExtensionType::Reserved] {
        let mut runtime_manager = TestRuntimeManager::new();
        let UploadTest {
            aggregator,
            clock,
        ...
        let report_public = create_report_custom(
            ...
            &hpke_keypair,
            /* public */
            Vec::from([Extension::new(extension_type, Vec::new())]),
            /* leader */ Vec::new(),
            /* helper */ Vec::new(),
        );
