...
use janus_core::{
    ...
    test_util::{
        install_test_trace_subscriber, run_vdaf, runtime::TestRuntimeManager,
    },
    ...
    let want_report_aggregation = ReportAggregation::<0, dummy::Vdaf>::new(
        ...
        *report.metadata().time(),
        ...
        ReportAggregationState::LeaderPollContinue {
            leader_state: transcript.leader_prepare_transitions[1]
                .transition
                .clone()
                .unwrap(),
        },
...
async fn leader_async_aggregation_job_init_poll_to_pending() {
    ...
    let lease = ds
        .run_unnamed_tx(|tx| {
            let task = leader_task.clone();
            let report = report.clone();
            let leader_state = transcript.leader_prepare_transitions[0]
                .prepare_state()
                .clone();
...
async fn leader_async_aggregation_job_continue_poll_to_pending() {
    ...
    let lease = ds
        .run_unnamed_tx(|tx| {
            let task = leader_task.clone();
            let report = report.clone();
            let leader_state = transcript.leader_prepare_transitions[1]
                .transition
                .clone()
                .unwrap();
