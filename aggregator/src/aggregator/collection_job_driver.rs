                ...
                Box::pin(async move {
                    ...
                    tx.put_batch_aggregation(
                        &BatchAggregation::<0, TimeInterval, dummy::Vdaf>::new(
                            *task.id(),
                            Interval::single(clock.now().to_time(
            ...
            Box::pin(async move {
                ...
                assert_matches!(collection_job.state(), CollectionJobState::Finished{ report_count, client_timestamp_interval, encrypted_helper_aggregate_share, leader_aggregate_share } => {
                    ...
                    assert_eq!(
                        ...
                        &Interval::new(
                            clock.now().to_time(&janus_messages::taskprov::TimePrecision::from_seconds(1)),
                            Duration::from_seconds(3 * time_precision.as_seconds(), &TimePrecision::from_seconds(1)),
                        ).unwrap()
                    );
