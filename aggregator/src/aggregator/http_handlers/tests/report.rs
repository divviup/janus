...
async fn upload_handler() {
    ...
    for _ in 0..2 {
        let mut response = handler
            .clone()
            .oneshot(
                Request::post(task.report_upload_uri().unwrap().path())
    ...
    let mut response = handler
        .clone()
        .oneshot(
            Request::post(task_end_soon.report_upload_uri().unwrap().path())
                .header("content-type", UploadRequest::MEDIA_TYPE)
                .body(Body::from(
...
async fn upload_handler_error_fanout() {
    ...
    // Use axum::serve instead of trillium_testing so we can send requests in parallel and
