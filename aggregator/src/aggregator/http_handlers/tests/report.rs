...
use janus_messages::{
    ...
    PlaintextInputShare, Report, ReportError, ReportId, ReportMetadata, Role, TaskDuration,
...
async fn upload_handler() {
    ...
    .with_time_precision(TaskDuration::from_seconds(1000))
...
async fn upload_handler_task_not_started() {
    ...
    let time_precision = TaskDuration::from_seconds(1000);
    ...
    .with_task_start(Some(
        clock
            ...
            .add_task_duration(&time_precision) // Add one time precision interval
            .unwrap()
            .add_task_duration(&time_precision) // Add another to be clearly in the future
            .unwrap(),
    ))
