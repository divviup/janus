...
use janus_messages::{
    ...
    PrepareResp, PrepareStepResult, Report, ReportError, ReportIdChecksum, Role, TaskId,
};
use opentelemetry::{
...
impl VdafOps {
        ...
        let (response, counters) = datastore
            .run_tx("aggregate_continue", |tx| {
                ...
                Box::pin(async move {
                    ...
                    // aggregations.
                    // XXX: need to clear last_prep_resp so that it does not leak across steps?
    ...
    where
        ...
        A::OutputShare: Send + Sync,
    {
        // XXX: do we need to check aggregation job state before beginning? (i.e. Helper{Init,Continue} rather than Helper{Init,Continue}Processing) -- if so, need to be sure not to break idempotency logic
        ...
        // Write accumulated aggregation values back to the datastore.
    ...
    where
        ...
        A::OutputShare: Send + Sync,
    {
        // XXX: consider writing task aggregation counter in this function, instead of returning it
