...
use crate::{
    ...
    message::{
        AggregationJobId, Duration, Extension, HpkeCiphertext, HpkeConfig, Nonce, Report,
        ReportShare, Role, TaskId, Time,
    },
    task::{AggregatorAuthKey, TaskParameters},
};
use prio::{
...
impl Transaction<'_> {
    ...
    /// If `task_id` is `None`, it is used. Otherwise the task ID is read from
    /// the row.
    fn task_parameters_from_row(
        ...
        let task_id = task_id.map_or_else(
            || {
                let encoded_task_id: Vec<u8> = row.get("id");
    ...
    pub(crate) async fn get_tasks(&self) -> Result<Vec<TaskParameters>, Error> {
        let stmt = self
            .tx
            .prepare_cached(
                "SELECT id, aggregator_role, aggregator_endpoints, vdaf, vdaf_verify_param,
                max_batch_lifetime, min_batch_size, min_batch_duration, tolerable_clock_skew,
                collector_hpke_config, agg_auth_key, hpke_config, hpke_private_key
                FROM tasks",
            )
    ...
    pub async fn get_client_report(&self, id: i64) -> Result<Report, Error> {
        ...
        let nonce_rand = row.get_bytea_as_u64("nonce_rand")?;
...
pub mod models {
    use super::Error;
    use crate::message::{AggregationJobId, Role, TaskId, TransitionError};
...
mod tests {
    ...
    use crate::{
        ...
        message::{ExtensionType, HpkeConfigId, Role, TransitionError},
        task::Vdaf,
        ...
        let (got_task_id, got_nonce, got_extensions, got_input_shares) = ds
            .run_tx(|tx| {
                Box::pin(async move {
                    ...
                    let nonce_time = Time::from_naive_date_time(row.get("nonce_time"));
                    let nonce_rand = row.get_bytea_as_u64("nonce_rand").unwrap();
                    let nonce = Nonce {
                        time: nonce_time,
                        rand: nonce_rand,
                    };
