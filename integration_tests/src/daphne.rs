...
impl<'a> CopyLogs for Daphne<'a> {
    fn logs<P: AsRef<Path>>(&self, destination: &P) {
        let host_destination_directory =
            destination
                .as_ref()
                .join(format!("{}-{}", self.role, self.daphne_container.id()));

        create_dir_all(&host_destination_directory).unwrap();

        let child_stdout_file =
            File::create(host_destination_directory.join("stdout.log")).unwrap();
        let child_stderr_file =
            File::create(host_destination_directory.join("stderr.log")).unwrap();
